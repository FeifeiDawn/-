...
def run_core_simulation(algo_p, fast_p, air_p, actual_d, fore_d, eval_base_opt, order_base_opt, order_static_val, review_p, init_inv, lt_array, fast_lt_array, ss_w, lb_w, ub_w):
    history = []
    inventory = init_inv 
    ...
    for t in range(1, TOTAL_WEEKS + 1):
        ...
        current_fast_lt = fast_lt_array[t]
        ...
        if "当周实际" in eval_base_opt:
            ...
            current_eval_base = np.mean(actual_d[t:end_t])
        current_eval_base = max(current_eval_base, 1)
        ...
        # ========================================================
        # 确知视野 (Visibility Window)：只能看到未来3周内确定的船期
        # ========================================================
        firm_pipeline = [item for item in pipeline if item['eta'] <= t + 3]

        # --- 第一道防线：快船救火守护进程 (LT=4) ---
        fast_qty_pieces = 0
        fast_penalty = 0
        if fast_p['enabled'] and t < TOTAL_WEEKS - 3:
            # 推演4周后的库存：由于视野只有3周，所以用firm_pipeline替代全量pipeline
            expected_inv_t4 = inventory - (proj_future_demand * 4) + sum(item['qty'] for item in firm_pipeline)
            trigger_target_fast = fast_p['trigger_wos'] * proj_future_demand
            
            if expected_inv_t4 < trigger_target_fast:
                replenish_target_fast = fast_p['replenish_wos'] * proj_future_demand
                fast_qty_pieces = max(replenish_target_fast - expected_inv_t4, fast_p['moq'])
                pipeline.append({'eta': t + current_fast_lt, 'qty': fast_qty_pieces, 'type': 'FAST'})
                fast_penalty = fast_qty_pieces * fast_p['unit_penalty']

        # --- 第二道防线：空运救火守护进程 (LT=1) ---
        air_qty_pieces = 0
        air_penalty = 0
        if air_p['enabled'] and t < TOTAL_WEEKS:
            expected_inv_t1 = inventory - proj_future_demand + sum(item['qty'] for item in firm_pipeline if item['eta'] == t + 1)
            trigger_target_air = air_p['trigger_wos'] * proj_future_demand
            
            if expected_inv_t1 < trigger_target_air:
                replenish_target_air = air_p['replenish_wos'] * proj_future_demand
                
                # 核心防积压抵扣逻辑：如果在t+2和t+3有确定的货要到，空运就不需要发那么多
                incoming_t2_t3 = sum(item['qty'] for item in firm_pipeline if item['eta'] in [t + 2, t + 3])
                
                # 理想情况需补缺口
                ideal_air_qty = replenish_target_air - expected_inv_t1 - incoming_t2_t3
                # 底线：至少要发够数量，确保t+1这一周活下来（不跌破触发线）
                min_survival_qty = trigger_target_air - expected_inv_t1
                
                # 最终空运量在 理想值、底线存活值、MOQ 中取最大
                air_qty_pieces = max(ideal_air_qty, min_survival_qty, air_p['moq'])
                
                pipeline.append({'eta': t + 1, 'qty': air_qty_pieces, 'type': 'AIR'})
                air_penalty = air_qty_pieces * air_p['unit_penalty'] 
        ...
        # 加上空运和快船的惩罚分
        score = air_penalty + fast_penalty + stockout_penalty + understock_penalty + overstock_penalty
        ...
        history.append({
            ...
            'Penalty_Over': overstock_penalty, 'Penalty_Air': air_penalty, 'Penalty_Fast': fast_penalty
        })
...
# 新增快船波动区间
fast_lt_min, fast_lt_max = sidebar.slider("快船周期三角分布区间 (周)", min_value=3, max_value=6, value=(3, 6))
...
sidebar.subheader("E. 紧急快船机制 (中坚防线)")
st.sidebar.info("预判4周后水位不足时强制快船。注意：触发阈值和补齐目标已交由 AI 自动寻优探索，您只需设定基础惩罚。")
enable_fast = sidebar.checkbox("🚢 启用快船救火机制", value=True)
fast_moq = sidebar.number_input("快船起订量 (件)", value=50, step=10, disabled=not enable_fast)
fast_unit_penalty = sidebar.number_input("单件快船罚分", value=0.5, step=0.1, format="%.2f", disabled=not enable_fast)
# 界面传参将用于占位，实际寻找的最优解在寻优网格内被接管
fast_params = {'enabled': enable_fast, 'trigger_wos': 2.0, 'replenish_wos': 4.0, 'moq': fast_moq, 'unit_penalty': fast_unit_penalty}

sidebar.subheader("F. 极限空运机制 (最后底牌)")
st.sidebar.info("预判下周不足时强制空运，并扣除未来3周可见在途避免积压。注意：触发阈值和补齐目标同样交由 AI 探索。")
...
# 界面传参将用于占位，实际寻找的最优解在寻优网格内被接管
air_params = {'enabled': enable_air, 'trigger_wos': 1.0, 'replenish_wos': 2.5, 'moq': air_moq, 'unit_penalty': air_unit_penalty}
...
fast_lt_arr = np.round(np.random.triangular(fast_lt_min, 4, fast_lt_max, TOTAL_WEEKS + 1)).astype(int)
fast_lt_arr = np.maximum(fast_lt_arr, 1)
...
current_env_state = (
    enable_crisis_d, crisis_week_d, crisis_pct_change, demand_noise, lt_min, lt_max, fast_lt_min, fast_lt_max,
    forecast_mode_ui, ma_n_ui, ema_beta_ui, static_forecast_val_ui, initial_inventory, moq_w_ui, 
    enable_fast, fast_moq, fast_unit_penalty,
...
st.info("系统将在设定的环境下执行**数千次**沙盘推演，并自动输出总成本最低的【海空快船联运最优配置】。在现代 CPU 下，这仅需数秒。")

with st.expander("💡 了解 AI 三栖联运决策推演逻辑"):
    st.markdown("""
    1. **设定不变环境**：系统将左侧市场波动、物流阻碍以及预测模型等作为不可更改的外部环境固定下来。
    2. **遍历海运策略空间 (底座)**：网格搜索发货频率 `RP`、安全系数 `Z`、缺口平滑系数 `α`、停发熔断线。
    3. **遍历快船救火空间 (中坚)**：如果开启了快船，AI 额外搜索最佳的**快船触发线**（1.5周, 2.0周等）和**快船补齐目标**（3.0周, 4.0周）。
    4. **遍历空运救火空间 (底牌)**：如果开启了空运，AI 同样搜索其独立的触发线与补齐目标。
    5. **最低罚分裁决**：对比每次推演产生的总惩罚分（断货罚分 + 压货罚分 + 快船空运单件成本），找出全局最低损益的商业平衡解。
    """)

if st.button("🌟 启动 AI 海空快船联合沙盘推演", type="primary"):
    with st.spinner("AI 网络搜索引擎高负荷运算中..."):
        ...
        # 快船网格：动态接管
        if fast_params['enabled']:
            grid_fast_t = [1.5, 2.0, 2.5]       
            grid_fast_r = [3.0, 4.0, 5.0]       
        else:
            grid_fast_t = [2.0]
            grid_fast_r = [4.0]
        ...
        if air_params['enabled']:
            grid_air_t = [1.0, 1.5, 2.0]        
            grid_air_r = [2.0, 3.0, 4.0]        
        else:
            grid_air_t = [1.0] 
            grid_air_r = [2.5] 
            ...
            for a in grid_alpha:
                for z in grid_z:
                    for trig in grid_trigger:
                        for f_t in grid_fast_t:
                            for f_r in grid_fast_r:
                                if fast_params['enabled'] and f_r <= f_t:
                                    continue
                                ...
                                for a_t in grid_air_t:
                                    for a_r in grid_air_r:
                                        ...
                                        current_fast_p = fast_params.copy()
                                        current_fast_p['trigger_wos'] = f_t
                                        current_fast_p['replenish_wos'] = f_r
                                        ...
                                        df_t, s_t = run_core_simulation(
                                            a_p, current_fast_p, current_air_p, actual_demand_arr, fore_arr, 
                                            eval_base_opt, order_base_opt, order_static_val, 
                                            rp, initial_inventory, lt_arr, fast_lt_arr, ss_w, lower_bound_w, upper_bound_w
                                        )
                                        ...
                                        if s_t < best_score or (s_t == best_score and inv_avg < best_avg_inv):
                                            ...
                                            best_params = {'rp':rp, 'alpha':a, 'z':z, 'trig':trig, 
                                                           'fast_t': f_t, 'fast_r': f_r,
                                                           'air_t': a_t, 'air_r': a_r}
                                            best_df = df_t
...
if 'ai_res' in st.session_state:
    ...
    msg = f"**✅ 联合寻优完成！最优海运策略 (总罚分 {total_score:.1f})**: 每 `{bp['rp']}` 周 1 发 | 平滑 `α={bp['alpha']}` | 最佳海运安全系数 `Z={bp['z']}`"
    if fast_params['enabled']:
        msg += f"\n\n**🚢 最优快船策略**: 预期(4周后)跌破 `{bp['fast_t']}`周 时触发 | 补足至 `{bp['fast_r']}`周"
    if air_params['enabled']:
        msg += f"\n\n**✈️ 最优空运策略**: 预期(下周)跌破 `{bp['air_t']}`周 时触发 | 补足至 `{bp['air_r']}`周"
    st.success(msg)
else: 
...
with col_popover:
    with st.popover("📜 查看当前评分规则 (按物理件数核算)"):
        ...
        🚢 **快船成本**：每发 1 件快船，扣 `{fast_params['unit_penalty']}` 分
...
cols[2].metric("快船件数总计", f"{int(df_res['Fast_Qty'].sum())} 件")
cols[3].metric("严重断货发生周数", f"{len(df_res[df_res['Is_Stockout']])} 周")
cols[4].metric("压货扣分期", f"{len(df_res[df_res['Penalty_Over'] > 0])} 周")
cols[5].metric("免罚分健康期", f"{len(df_res[df_res['Score'] == 0])} 周")
...
if fast_params['enabled']:
    fig.add_trace(go.Bar(x=df_res['Week'], y=df_res['Fast_Qty'], name="🚢 快船救火", marker_color='#1E90FF', hovertemplate="快船补货: %{y:.0f}件"), row=1, col=1)
